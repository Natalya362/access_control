Процедура ПередЗаписью(Отказ)
	...
	Запрос.Текст = 
		"ВЫБРАТЬ ПЕРВЫЕ 1
		|	ТочкиКонтроля.Ссылка
		|ИЗ
		|	Справочник.ТочкиКонтроля КАК ТочкиКонтроля
		|ГДЕ
		|	ТочкиКонтроля.Наименование = &Наименование
		|	И ТочкиКонтроля.Ссылка <> &ТекущаяСсылка
		|	И НЕ ТочкиКонтроля.ПометкаУдаления";
	...
	Если Не Результат.Пустой() Тогда
		Сообщить("Точка контроля с наименованием """ + ЭтотОбъект.Наименование + """ уже существует!");
		Отказ = Истина;
		Возврат;
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(ЭтотОбъект.Код) Тогда
		...
		Запрос.Текст = 
			...
			|	Справочник.ТочкиКонтроля КАК ТочкиКонтроля
			|ГДЕ
			|	НЕ ТочкиКонтроля.ПометкаУдаления";
	...
	Результат = Запрос.Выполнить();
	Если Не Результат.Пустой() Тогда
		Сообщить("Точка контроля с кодом """ + ЭтотОбъект.Код + """ уже существует!");
		Отказ = Истина;
		Возврат;
	КонецЕсли;
