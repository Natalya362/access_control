Процедура ПередЗаписью(Отказ)
	...
	Запрос.Текст = 
		"ВЫБРАТЬ ПЕРВЫЕ 1
		|	ПричиныПосещения.Ссылка
		|ИЗ
		|	Справочник.ПричиныПосещения КАК ПричиныПосещения
		|ГДЕ
		|	ПричиныПосещения.Наименование = &Наименование
		|	И ПричиныПосещения.Ссылка <> &ТекущаяСсылка
		|	И НЕ ПричиныПосещения.ПометкаУдаления";
	...
	Если Не Результат.Пустой() Тогда
		Сообщить("Причина посещения с наименованием """ + ЭтотОбъект.Наименование + """ уже существует!");
		Отказ = Истина;
		Возврат;
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(ЭтотОбъект.Код) Тогда
		...
		Запрос.Текст = 
			"ВЫБРАТЬ
			|	ПричиныПосещения.Код
			|ИЗ
			|	Справочник.ПричиныПосещения КАК ПричиныПосещения
			|ГДЕ
			|	НЕ ПричиныПосещения.ПометкаУдаления";
	...
	Результат = Запрос.Выполнить();
	Если Не Результат.Пустой() Тогда
		Сообщить("Причина посещения с кодом """ + ЭтотОбъект.Код + """ уже существует!");
		Отказ = Истина;
		Возврат;
	КонецЕсли;
