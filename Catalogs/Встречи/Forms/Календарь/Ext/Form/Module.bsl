...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ВариантПериода = ПолучитьВариантПериодаКалендаряВстреч();
...
Процедура ПланировщикПередСозданием(Элемент, Начало, Конец, Значения, Текст, СтандартнаяОбработка) 	
    ...
    ЗначенияЗаполнения.Вставить("Владелец", ПолучитьПользователяПоКоду(ТекущийПользовательБазы));  
...
Процедура ПланировщикПриОкончанииРедактирования(Элемент, НовыйЭлемент, ОтменаРедактирования)
    ...
    Для Каждого ВыделенныйЭлемент Из Элемент.ВыделенныеЭлементы Цикл
        ...
        ОбрабатываемыйЭлемент.Вставить("Владелец", ПолучитьПользователяПоКоду(ТекущийПользовательБазы)); 
...
Процедура УстановитьОтображениеПланировщика()
	...
	Если ВариантПериода = "День" Тогда
		...
		Планировщик.ОтступСНачалаПереносаШкалыВремени = 7; // Изменено с 9 на 7 (с 7:00)
		Планировщик.ОтступСКонцаПереносаШкалыВремени = 5;  // Изменено с 6 на 5 (до 19:00)
	...
	ИначеЕсли ВариантПериода = "Неделя" Тогда
		...
		Планировщик.ЕдиницаПериодическогоВарианта = ТипЕдиницыШкалыВремени.Час;
		Планировщик.КратностьПериодическогоВарианта = 24;
		Планировщик.ОтступСНачалаПереносаШкалыВремени = 7; 
...
Процедура ПолучитьКодТекущегоПользователя()
    ...
    ПользовательСсылка = Справочники.Пользователи.НайтиПоНаименованию(ТекущийПользователь.Имя);
    Если ЗначениеЗаполнено(ПользовательСсылка) Тогда
        ТекущийПользовательБазы = ПользовательСсылка.Код;
    Иначе
        ТекущийПользовательБазы = ТекущийПользователь.Имя;
    КонецЕсли;
КонецПроцедуры
...
Функция ПолучитьПользователяПоКоду(КодПользователя)    
    ...
    Пользователь = Справочники.Пользователи.НайтиПоКоду(КодПользователя);
    Если Не ЗначениеЗаполнено(Пользователь) Тогда
        Пользователь = Справочники.Пользователи.НайтиПоНаименованию(КодПользователя);
    КонецЕсли;
...
Процедура ОбновитьДанныеПланировщикаСервер(ПлавнаяПрокрутка = Ложь)
	...
	// Загружаем только встречи текущего пользователя
	Владелец = Справочники.Пользователи.НайтиПоКоду(ТекущийПользовательБазы);
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
...
Процедура ОткрытьФормуТекущегоЭлементаПланировщика()
	...
	Если Элементы.Планировщик.ВыделенныеЭлементы.Количество() = 0 Тогда
		Сообщить(НСтр("ru = 'Не выбрана ни одна встреча для редактирования.'", "ru"));
		Возврат;
	КонецЕсли;
...
&НаСервере
Процедура ЗаписатьВКалендарь(ДатаНачала, ДатаКонца, Наименование)  
	
	Запись = Справочники.Встречи.СоздатьЭлемент();
    Запись.Владелец = Справочники.Пользователи.НайтиПоКоду(ТекущийПользовательБазы);
...
Процедура ПриИзмененииВариантПериода()
	...
	СохранитьВариантПериодаКалендаряВстреч(ВариантПериода);
